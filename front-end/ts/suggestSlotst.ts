...
export function getCharacteristics(
  ...
  characteristicsTableElement: HTMLDivElement,
  tabledata: TableRow[]
): any[] {
  ...
  const criteriaInputs = getCriteriaInputs(
    table,
    mainDiv,
    "criteria-container-characteristics"
  );
...
export function createHtmlElements(): void {
  ...
  const replacementClassCriteriaContainer: HTMLDivElement =
    document.querySelector("#ReplacementClassCriteria") as HTMLDivElement;
  const replacementClassContainer = document.getElementById(
    "ReplacementClass"
  ) as HTMLDivElement;

  //Criação do botão para sugerir slots as aulas de substituição
  const suggestSlotReplaceButton: HTMLButtonElement =
    document.createElement("button");
  suggestSlotReplaceButton.textContent =
    "Sugerir slots para alocação da aula de substituição";
   ...
   */
  showCriteriaSuggestSlots(
    replacementClassCriteriaContainer,
    document.getElementById(
      "ReplacementClassCharacteristicsTable"
    ) as HTMLDivElement,
    document.getElementById("ReplacementClassTimeTable") as HTMLDivElement
  );
  replacementClassContainer.style.display = "none";
  document
    .getElementById("SuggestSlots")
    ?.insertBefore(
      suggestSlotReplaceButton,
 ...
 */
export function showCriteriaSuggestSlots(
  mainDiv: HTMLDivElement,
  characteristicsTableElement: HTMLDivElement,
  timeTableElement: HTMLDivElement
): void {
  const buttonAddNewCriteriaDivTimeTable: HTMLButtonElement =
    document.createElement("button");
  buttonAddNewCriteriaDivTimeTable.textContent = "Or";
  const timeTableContainer = mainDiv.querySelector(
    ".ContainerTimeTable"
  ) as HTMLDivElement;
  buttonAddNewCriteriaDivTimeTable.addEventListener("click", () =>
    addNewCriteriaContainer(
      timeTableContainer,
      buttonAddNewCriteriaDivTimeTable,
      true
    )
  );
  ...
  createCriteriaContainer(
    ...
    buttonAddNewCriteriaDivTimeTable,
    true
  ).prepend(createSelectWithOptionsToClassDuration());

  const buttonAddNewCriteriaDivCharacteristics: HTMLButtonElement =
    document.createElement("button");
  buttonAddNewCriteriaDivCharacteristics.textContent = "Or";
  const characteristicsContainer = mainDiv.querySelector(
    ".ContainerCharacteristics"
  ) as HTMLDivElement;
  buttonAddNewCriteriaDivCharacteristics.addEventListener("click", () =>
    addNewCriteriaContainer(
      characteristicsContainer,
      buttonAddNewCriteriaDivCharacteristics,
      false
    )
  );
 ...
 */
export function addNewCriteriaContainer(
  mainDiv: HTMLDivElement,
  buttonAddNewCriteriaContainer: HTMLButtonElement,
  isRooms: boolean
): void {
  const criteriaContainer: HTMLDivElement = createCriteriaContainer(
    mainDiv,
    buttonAddNewCriteriaContainer,
    isRooms
  );
  ...
  criteriaContainer.appendChild(
    createRemoveButton(
      mainDiv,
      criteriaContainer,
      criteriaContainer.className.split(" ")[1],
 ...
 */
export function createCriteriaContainer(
  mainDiv: HTMLDivElement,
  buttonAddNewCriteriaContainer: HTMLButtonElement,
  isRooms: boolean
): HTMLDivElement {
  ...
  if (isRooms)
    criteriaContainer.className =
      "criteria-container criteria-container-timeTable";
  else
    criteriaContainer.className =
      "criteria-container criteria-container-characteristics";
  ...
  const buttonAddNewCriteria: HTMLButtonElement = createNewCriteriaButton(
    mainDiv,
    criteriaContainer,
    isRooms
  );
  criteriaContainer.appendChild(buttonAddNewCriteria);
  const element: HTMLDivElement = addNewCriteriaOptionToSuggestSlots(
    mainDiv,
    criteriaContainer.className.split(" ")[1],
    isRooms
  );
 ...
 */
function createNewCriteriaButton(
  mainDiv: HTMLDivElement,
  container: HTMLDivElement,
  isRooms: boolean
): HTMLButtonElement {
  const buttonAddNewCriteria: HTMLButtonElement =
    document.createElement("button");
  ...
  buttonAddNewCriteria.addEventListener("click", () => {
    ...
    const criteriaContainerComponents: HTMLDivElement =
      addNewCriteriaOptionToSuggestSlots(
        mainDiv,
        container.className.split(" ")[1],
        isRooms
      );
    criteriaContainerComponents.appendChild(
      createRemoveButton(
        mainDiv,
        criteriaContainerComponents,
        container.className.split(" ")[1],
 ...
 */
function addNewCriteriaOptionToSuggestSlots(
  mainDiv: HTMLDivElement,
  criteriaContainerName: string,
  isRooms: boolean
): HTMLDivElement {
  ...
  criteriaContainerComponents
    .querySelector(".criteria-column-container")
    ?.appendChild(
      createSelectWithOptionsToColumns(criteriaContainerComponents, isRooms)
    );
  //
 ...
 */
function createSelectWithOptionsToColumns(
  criteriaContainerComponents: HTMLDivElement,
  isRooms: boolean
): HTMLSelectElement {
...
function showExtraOptions(
  ...
  const extraOptions: NodeListOf<HTMLOptionElement> =
    criteriaContainerComponents.querySelectorAll(".extra-options");
  let display: string;
...
function createRemoveButton(
  mainDiv: HTMLDivElement,
  criteriaContainer: HTMLDivElement,
  criteriaContainerName: string,
  className: string,
 ...
 */
export function getCriteriaInputs(
  table: Tabulator,
  mainDiv: HTMLDivElement,
  criteriaContainerName: string
): { usedColumns: string[]; finalFilter: string } {
  let finalFilter: string = "";
  let usedColumns: string[] = [];
  table.clearFilter();
  const filter: NodeListOf<Element> = mainDiv.querySelectorAll(
    "." + criteriaContainerName
  );
  for (let i = 0; i != filter.length; i++) {
    let criteriaString: string = "";
    const criteriaContainerComponents = filter[i].querySelectorAll(
      ".criteria-container-components"
    );
    for (let i = 0; i != criteriaContainerComponents.length; i++) {
      const column: HTMLSelectElement = criteriaContainerComponents[
        i
      ].querySelector(".criteria-column-selector") as HTMLSelectElement;
      const operator: HTMLSelectElement = criteriaContainerComponents[
        i
      ].querySelector(".criteria-filter-option-selector") as HTMLSelectElement;
      const input: HTMLInputElement = criteriaContainerComponents[
        i
      ].querySelector(".criteria-input") as HTMLInputElement;
      ...
      if (criteriaString == "")
        criteriaString += `data["${columnValue}"] ${operator.value} "${inputValue}"`;
      else
        criteriaString += ` && data["${columnValue}"] ${operator.value} "${inputValue}"`;
    }
    if (criteriaString == "") continue;
    if (i != filter.length - 1)
      finalFilter += "( " + criteriaString + " )" + " || ";
...
function generateSugestions(
  ...
  Object.keys(characteristics).forEach((room) => {
    for (let j = 0; j != timeTable.length; j++) {
      const object = {
        "Sala atribuída à aula": room,
        "Data da aula": timeTable[j].day,
        "Dia da semana": timeTable[j].dayWeek,
        "Hora início da aula": timeTable[j].startHour + ":00",
        "Hora fim da aula": timeTable[j].endHour + ":00",
      };
      const finalObject = Object.assign(object, {
        "Sala atribuída à aula": room,
      });
      suggestions[room + timeTable[j].day + timeTable[j].endHour + ":00"] =
        finalObject;
    }
  });
...
function removeConflicts(suggestions: any, table: Tabulator): any {
  ...
  table.getRows(true).forEach((row: any) => {
    const s =
      row.getData()["Sala atribuída à aula"] +
      row.getData()["Data da aula"] +
      row.getData()["Hora fim da aula"];
    conflitData[s] = {};
  });
  ...
  Object.values(suggestions).forEach((data: any) => {
    ...
    while (true) {
      ...
      const key: string =
        data["Sala atribuída à aula"] + data["Data da aula"] + time;
      if (!(key in conflitData))
        if (key in suggestions) rowsWithoutConflicts.push(suggestions[key]);
...
function daysBasedOnFilter(mainDiv: HTMLDivElement): Date[] {
  ...
  const filter: NodeListOf<Element> = mainDiv.querySelectorAll(
    ".criteria-container-timeTable"
  );
  for (let j = 0; j != filter.length; j++) {
    const criteriaContainerComponents = filter[j].querySelectorAll(
      ".criteria-container-components"
    );
    ...
    for (let i = 0; i != criteriaContainerComponents.length; i++) {
      ...
      const inputValue: HTMLInputElement = criteriaContainerComponents[
        i
      ].querySelector(".criteria-input") as HTMLInputElement;
    ...
    daysToExclude.length
      ? daysToInclude.push(
          ...daysToIncludeAux.filter((element) =>
            daysToExclude.every(
              (excludedDay) => excludedDay.getTime() !== element.getTime()
            )
          )
        )
...
function hoursBasedOnFilter(
  ...
  const durationElement = mainDiv.querySelector(
  ...
  for (let j = 0; j != filter.length; j++) {
    ...
    for (let i = 0; i != criteriaContainerComponents.length; i++) {
      ...
      if (
        column.value !== "Hora início da aula" ||
        new Date(`1 ${inputValue.value}`) > new Date("1 22:00") ||
        new Date(`1 ${inputValue.value}`) < new Date("1 08:00")
      )
        continue;
      if (operator.value === "==") hoursFromAnd.push(inputValue.value);
      else if (operator.value === "!=") hoursToExclude.push(inputValue.value);
      else if (operator.value === "<=")
        finalHour = new Date(`1 ${inputValue.value}`);
      else initialHour = new Date(`1 ${inputValue.value}`);
    }

    if (hoursFromAnd.length < 1) {
      hoursFromAnd.push(
        ...getClassesStartingHours(initialHour, finalHour, classDuration)
      );
    }
    hours.push(
      ...hoursFromAnd.filter((hour) => hoursToExclude.indexOf(hour) < 0)
    );
  }
  return [...new Set(hours)].map((startingHour) => {
    ...
    return {
      ...
      endHour: `${String(endingHourDate.getHours()).padStart(2, "0")}:${String(
        endingHourDate.getMinutes()
      ).padStart(2, "0")}`,
    };
  });
}
 ...
 */
function getFilteredDateHourCombination(
  mainDiv: HTMLDivElement
): { day: string; dayWeek: string; startHour: string; endHour: string }[] {
  const combinations: {
    day: string;
    dayWeek: string;
    startHour: string;
  ...
  const hours: { startHour: string; endHour: string }[] =
    hoursBasedOnFilter(mainDiv);
  daysBasedOnFilter(mainDiv).forEach((day) => {
    hours.forEach((hour) =>
      combinations.push(
        Object.assign(
          { day: formatDateToDDMMYYYY(day), dayWeek: getDayOfWeek(day) },
          hour
        )
      )
    );
